...
function bindRecentThumbnailActions(dragEl, filePath) {
...
function renderRecents() {
  ...
  recents.forEach(item => {
    ...
    if (videoId) {
      ...
      thumbnailHtml = `<div class="recent-thumbnail" style="background: linear-gradient(45deg, #f09433 0%, #e6683c 25%, #dc2743 50%, #cc2366 75%, #bc1888 100%); color: white; border: none;">
...
async function loadGifSource(filePath) {
...
function updateGifSliderUI() {
  if (gifDuration > 0 && gifSliderRange) {
    const startPct = (gifStartVal / gifDuration) * 100;
    const endPct = (gifEndVal / gifDuration) * 100;
    const widthPct = Math.max(0, endPct - startPct);
...
function seekGifVideoPlayer(time) {
