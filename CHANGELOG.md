...
### Fixed
- Fixed GitHub Actions / `npm test` failing on Windows when npm@latest blocks Electron install scripts: moved `extractStreamUrls` into pure `src/main/clipper-streams.js` so unit tests no longer load Electron, and allowed `electron` / `electron-winstaller` install scripts in `package.json`.
- Re-release of the v1.9.4 clipper preview and download-complete card fixes (the `v1.9.4` tag publish never completed because CI failed before packaging).
